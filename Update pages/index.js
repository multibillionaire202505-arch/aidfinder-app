...
const ALL = [
  ...
  { category:"Income", link:"https://www.sba.gov/funding-programs",
    i18n:{ en:{ title:"SBA Small Business Programs", desc:"Loans, counseling & resources for entrepreneurs." },
           fr:{ title:"Programmes SBA", desc:"Prêts, counseling & ressources pour entrepreneurs." },
...
export default function Home() {
  ...
  // ✅ SSR-safe native share detection
  ...
  useEffect(()=>{ try{ const raw = localStorage.getItem("aidfinder_favs"); if(raw) setFavs(JSON.parse(raw)); }catch{} },[]);
  useEffect(()=>{ try{ localStorage.setItem("aidfinder_favs", JSON.stringify(favs)); }catch{} },[favs]);
  ...
  const doNativeShare = async (p) => {
    if (!canNativeShare) { setShareOpenModal(true); return; }
    try {
  ...
  return (
    <>
                ...
                <div className="cardActions">
                  ...
                  <div className="menuWrap" onClick={(e)=>e.stopPropagation()}>
                    <button
                      type="button"
                      className="secondary"
                      onClick={()=> canNativeShare ? doNativeShare(p) : setShareOpenIndex(shareOpenIndex===i? null : i)}
                      aria-haspopup="menu"
      ...
      <style jsx global>{`
        ...
        :root[data-theme="dark"] .chipActive{ background:#0f2a22; border-color:#0e3527; color:#c6f0dc; }

        .apply{ background:#111827; color:#fff; border:0; border-radius:10px; padding:10px 14px; text-decoration:none; font-weight:700; }
        :root[data-theme="dark"] .apply{ background:#e5e7eb; color:#0b1220; }
